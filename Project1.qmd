...
#step 1 and 2
 ...
 #step 3
    mutate(Year=as.character(1900+as.numeric(substr(Data_1$Enrollment, 8, 9)))) |>
    mutate(Course_Year=paste0(substr(Enrollment,1,7),Year)) |>
    collect()
  
#step 5
    County<-Data[grep(pattern = ", \\w\\w", Data$area_name),] |>
    mutate(State_abbr=trimws(substr(area_name, nchar(area_name)-2, nchar(area_name)))) |>
      collect()
    class(County) <- c("County", class(County))
